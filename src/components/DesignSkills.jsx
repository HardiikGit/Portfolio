import '../css/DesignSkills.css'

export default function Skills() {
     return (
          <div className="MainSkillSection">
               <div className="container">
                    <div className='Skills-Content'>
                         <div className="row gap40">
                              <div className="col-lg-6 col-sm-12">
                                   <div className='Sub-Heading' data-aos="fade-in">
                                        <h2 className='relative'>Design Skills</h2>
                                   </div>
                                   <div className="Skills-Boxb" data-aos="fade-in">
                                        <div className="Skill-1">
                                             <div className="Skill-name d-flex align-items-center justify-space-between">
                                                  <h3>Ui/Ux Designs</h3>
                                                  <p>75%</p>
                                             </div>
                                             <div className="Percenteage-box">
                                                  <div className="skill-Percentage"></div>
                                             </div>
                                        </div>
                                        <div className="Skill-2">
                                             <div className="Skill-name d-flex align-items-center justify-space-between">
                                                  <h3>Webpage Designs</h3>
                                                  <p>90%</p>
                                             </div>
                                             <div className="Percenteage-box">
                                                  <div className="skill-Percentage"></div>
                                             </div>
                                        </div>
                                        <div className="Skill-3">
                                             <div className="Skill-name d-flex align-items-center justify-space-between">
                                                  <h3>Graphic Design</h3>
                                                  <p>70%</p>
                                             </div>
                                             <div className="Percenteage-box">
                                                  <div className="skill-Percentage"></div>
                                             </div>
                                        </div>
                                   </div>
                              </div>
                              <div className="col-lg-6 col-sm-12">
                                   <div className='Sub-Heading' data-aos="fade-in">
                                        <h2 className='relative'>Coding Skills</h2>
                                   </div>
                                   <div className="Skills-Box" data-aos="fade-in">
                                        <div className="Skill-4">
                                             <div className="Skill-name d-flex align-items-center justify-space-between">
                                                  <h3>HTML/CSS/Tailwind</h3>
                                                  <p>90%</p>
                                             </div>
                                             <div className="Percenteage-box">
                                                  <div className="skill-Percentage"></div>
                                             </div>
                                        </div>
                                        <div className="Skill-5">
                                             <div className="Skill-name d-flex align-items-center justify-space-between">
                                                  <h3>Java Script</h3>
                                                  <p>75%</p>
                                             </div>
                                             <div className="Percenteage-box">
                                                  <div className="skill-Percentage"></div>
                                             </div>
                                        </div>
                                        <div className="Skill-6">
                                             <div className="Skill-name d-flex align-items-center justify-space-between">
                                                  <h3>React Js</h3>
                                                  <p>65%</p>
                                             </div>
                                             <div className="Percenteage-box">
                                                  <div className="skill-Percentage"></div>
                                             </div>
                                        </div>
                                   </div>
                              </div>
                         </div>
                    </div>
               </div>
          </div>
     );
}